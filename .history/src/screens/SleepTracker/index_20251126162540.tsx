...
export default function SleepTracker() {
  ...
  const [bedPeriod, setBedPeriod] = useState<'AM' | 'PM'>('AM');
  ...
  const formatTimeInput = (text: string) => {
    ...
    // Clamp hour and minute ranges
    const parts = cleaned.split(':');

    if (parts[0]) {
      let hour = parseInt(parts[0], 10);
      if (hour > 23) hour = 23;
      parts[0] = hour.toString().padStart(2, '0');
    }

    if (parts[1]) {
      let min = parseInt(parts[1], 10);
      if (min > 59) min = 59;
      parts[1] = min.toString().padStart(2, '0');
    }
            ...
            <View style={styles.timeBox}>
              ...
              <View style={styles.timeInput}>
                <TextInput
                  ...
                  maxLength={5}
                  value={bedTime}
                  onChangeText={t => setBedTime(formatTimeInput(t))}
                />
