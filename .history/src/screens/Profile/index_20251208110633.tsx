      ...
      <View style={styles.section}>
        ...
        <View style={styles.weightTrendCard}>
          ...
          <View style={styles.graphContainer}>
            <WeightGraph data={[149.8, 150.2, 151.0, 150.5, 148.9]} />
...
const WeightGraph = ({ data }: { data: number[] }) => {
  const max = Math.max(...data);
  const min = Math.min(...data);

  // convert weight values into Y positions (0–100%)
  const normalized = data.map(v => ((max - v) / (max - min || 1)) * 80 + 10);
    ...
    <View style={styles.graphArea}>
      {normalized.map((y, idx) => (
        <View
          ...
          style={[
            styles.graphPoint,
            { left: `${(idx / (data.length - 1)) * 100}%`, top: `${y}%` },
          ]}
        />
      ))}
